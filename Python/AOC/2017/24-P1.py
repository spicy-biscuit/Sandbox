...
@cache
def Bridge(CurrentStrength, LastPort, RemainingPieces):
  NowhereLeft = True
  ...
  for Piece in RemainingPieces:
    if Piece[0] == LastPort:
      ...
      List.append(Bridge(CurrentStrength + Piece[0] + Piece[1], Piece[1], NewPieces))
    if Piece[1] == LastPort:
      NewPieces = RemainingPieces
      NewPieces = list(NewPieces)
      NewPieces.remove(Piece)
      NewPieces = tuple(NewPieces)
      List.append(Bridge(CurrentStrength + Piece[0] + Piece[1], Piece[0], NewPieces))
  return(max(List))

print(Bridge(0, 0, tuple(Input)))