...
for Instruction in Input:
  RegisterName = Instruction[:Instruction.find(" ")]
  Instruction = Instruction[Instruction.find(" ") + 1:]
  RegisterIndex = Registers.index(RegisterName)
  Increment = Instruction[:3]
  Instruction = Instruction[4:]
  ChangeBy = int(Instruction[:Instruction.find(" ")])
  Instruction = Instruction[Instruction.find("if") + 3:]
  CheckAgainst = Instruction[:Instruction.find(" ")]
  Instruction = Instruction[Instruction.find(" ") + 1:]
  CheckOperator = Instruction[:Instruction.find(" ")]
  Instruction = Instruction[Instruction.find(" ") + 1:]
  CheckNumber = int(Instruction)
  # print(RegisterName, Increment, ChangeBy, CheckAgainst, CheckOperator, CheckNumber)
  
  CheckAgainstIndex = Registers.index(CheckAgainst)
  CheckAgainstValue = Values[CheckAgainstIndex]
  Update = False
  if CheckOperator == "<":
    if CheckAgainstValue < CheckNumber:
      Update = True
  elif CheckOperator == "!=":
    if CheckAgainstValue != CheckNumber:
      Update = True
  ...
  if Update:
    if Increment == "inc":
      Values[RegisterIndex] = Values[RegisterIndex] + ChangeBy
    else:
      Values[RegisterIndex] = Values[RegisterIndex] - ChangeBy
