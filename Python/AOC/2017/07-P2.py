...
for x in Input:
  Names.append(x[:x.find(" ")])
  Data.append(x[x.find(" ") + 1:])

@cache
def FindWeight(Tower):
  TowerIndex = Names.index(Tower)
  TowerData = Data[TowerIndex]
  if TowerData.find("-") == -1:
    return(int(TowerData[TowerData.find("(") + 1:TowerData.find(")")]))
  TowerWeight = int(TowerData[TowerData.find("(") + 1:TowerData.find(")")])
  ChildTowers = TowerData[TowerData.find(">") + 1:]
  ChildTowers = ChildTowers.split(", ")
  for x in ChildTowers:
    if x[0] == " ":
      x = x[1:]
    TowerWeight += FindWeight(x)
  return(int(TowerWeight))

for Tower in Names:
  TowerData = Data[Names.index(Tower)]
  if TowerData.find(">") == -1:
    continue
  ChildTowers = TowerData[TowerData.find(">") + 2:]
  ChildTowers = ChildTowers.split(", ")
  Weights = []
  for x in ChildTowers:
    if x[0] == " ":
      x = x[1:]
    Weights.append(FindWeight(x))
  Common = Weights[0]
  for x in Weights:
    if x != Common:
      print(Weights)
...
print(Data[Names.index("ltleg")])

#cheated a lil, have to understand the problem to understand the end bit of code