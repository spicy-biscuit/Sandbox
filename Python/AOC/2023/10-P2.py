Input = ["""7.FF7777.|77.-L-77.|F-7-.L77|7F|7-|-JF77FFF777.7FL--F-F..F.-7-F7-J.FFF7JFF7-J7....FFJFL.|-L7F--|-L7L-|-|-|.F77F7F-|7---L-.7-.F-7F|-L77...-.-
...
for x in range(len(Input[0])):
  String = String + " "
Map = [String for x in range(len(Input))]
...
for x in range(len(Input)):
  if Input[x].find("S") != -1:
    Row = x
    Column = Input[x].find("S")
    break

#find what type of pipe the start is
StartUp = False
StartRight = False
StartDown = False
StartLeft = False
...
if Input[Row - 1][Column] == "7" or Input[Row - 1][Column] == "|" or Input[Row - 1][Column] == "F":
  StartUp = True
if Input[Row][Column + 1] == "7" or Input[Row][Column + 1] == "-" or Input[Row][Column + 1] == "J":
  StartRight = True
if Input[Row + 1][Column] == "J" or Input[Row + 1][Column] == "|" or Input[Row + 1][Column] == "L":
  StartDown = True
if Input[Row][Column - 1] == "F" or Input[Row][Column - 1] == "-" or Input[Row][Column - 1] == "L":
  StartLeft = True
...
#loop until get back to original spot
Counter = 0
OriginalDirection = ""
if StartUp:
...
while True:
  CurrentPipe = Input[Row][Column]
  if CurrentPipe == "S":
    CurrentPipe = StartType
  List = []
  if CurrentPipe in UpPipes:
    Up = True
    List.append("Up")
  if CurrentPipe in RightPipes:
    Right = True
    List.append("Right")
  if CurrentPipe in DownPipes:
    Down = True
    List.append("Down")
  if CurrentPipe in LeftPipes:
    Left = True
    List.append("Left")
    
  #P2 visualization
  Map[Row] = Map[Row][:Column] + CurrentPipe + Map[Row][Column + 1:]
  GoToNext = ""
  for x in List:
    if x != OriginalDirection:
      GoToNext = x
      List.remove(x)
      break
      
  RemainingDirection = List[0]
  if GoToNext == "Up":
    OriginalDirection = "Down"
  if GoToNext == "Right":
    OriginalDirection = "Left"
  ...
  #break statement
  if OriginalRow == Row and OriginalColumn == Column:
    break
...
OriginalWidth = len(Map2[0])
OriginalHeight = len(Map2)

String = ""
for x in range((OriginalWidth)):
  String = String + " "
Map2 = [String] + Map2 + [String]

for x in range(len(Map2)):
  Map2[x] = " " + Map2[x] + " "

for Row2 in range(1, OriginalHeight - 1):
  for Column2 in range(1, OriginalWidth - 1):
    if Map2[Row2][Column2] != " ":
      continue
    # '-' insert
    if Map2[Row2][Column2 - 1] in RightPipes and Map2[Row2][Column2 + 1] in LeftPipes:
      Map2[Row2] = Map2[Row2][:Column2] + "-" + Map2[Row2][Column2 + 1:]
    # '|' insert
    elif Map2[Row2 - 1][Column2] in DownPipes and Map2[Row2 + 1][Column2] in UpPipes:
      Map2[Row2] = Map2[Row2][:Column2] + "|" + Map2[Row2][Column2 + 1:]

#Search for all outsides
def CountOutsides():
  Count = 0
  for Row in range(len(Map2)):
    for Column in range(len(Map2[0])):
      if Map2[Row][Column] == "O":
        Count += 1
  return(Count)

Map2[1] = " " + "O" + Map2[1][2:] #virus search
Counter = 0
PenultimateCount = 0
PreviousCount = 0
CurrentCount = 1
OutsideCount = 1
AdjacentOutside = False
while True:
  Break = True
  for Row in range(1, len(Map2) - 1):
    for Column in range(1, len(Map2[0]) - 1):
      AdjacentOutside = False
      if Map2[Row][Column] == " ":
        AdjacentOutside = False
        if Map2[Row - 1][Column] == "O":
          AdjacentOutside = True
          OutsideCount += 1
          Break = False
        if Map2[Row][Column + 1] == "O":
          AdjacentOutside = True
          OutsideCount += 1
          Break = False
        if Map2[Row + 1][Column] == "O":
        ...
        if Map2[Row][Column - 1] == "O":
          ...
          OutsideCount += 1
          Break = False
        if AdjacentOutside == True:
          Map2[Row] = Map2[Row][:Column] + "O" + Map2[Row][Column + 1:]
  PenultimateCount = PreviousCount
  PreviousCount = CurrentCount
  CurrentCount = CountOutsides()
  if PreviousCount == CurrentCount and Break and PenultimateCount == CurrentCount:
    break
  Counter += 1
  if Counter % 10 == 0:
    print(Counter, OutsideCount)

print(Counter, OutsideCount)

#Search for all " " on even coords
RealCounter = 0
for Row2 in range(1, len(Map2) - 1):
  for Column2 in range(1, len(Map2[0]) - 1):
    if Row2 % 2 == 1 and Column2 % 2 == 0 and Map2[Row2][Column2] == " ":
      Map2[Row2] = Map2[Row2][:Column2] + "I" + Map2[Row2][Column2 + 1:]
      RealCounter += 1
print(RealCounter)
for x in Map2:
  print(x)
