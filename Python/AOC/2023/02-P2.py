...
for GAME in Input:
  ...
  for INSTANCE in GAME:
    ...
    for NUMBER in INSTANCE:
      if NUMBER[0] == " ":
        NUMBER = NUMBER[1:]
      numToCheck = int(NUMBER[:2])
      if NUMBER.find("red") != -1:
        if numToCheck > maxRed:
          maxRed = numToCheck
      if NUMBER.find("green") != -1:
        if numToCheck > maxGreen:
          maxGreen = numToCheck
      if NUMBER.find("blue") != -1:
        if numToCheck > maxBlue:
          maxBlue = numToCheck
  Total += maxRed * maxBlue * maxGreen
  print(GAME, maxRed * maxBlue * maxGreen)
print(Total)
