...
Width = len(Input[0])
New = [["." for  x in range(Width)]]
Input = New + Input + New
# print(Input)
for x in range(len(Input)):
  Input[x] = ["."] + Input[x] + ["."]

def First(Row, Column, RowChange, ColumnChange):
  while True:
    if Row < 1 or Row >= len(Input) - 1 or Column < 1 or Column >= len(Input[0]) - 1:
      return(False)
    Row += RowChange
    Column += ColumnChange
    if Input[Row][Column] in ["#"]:
      return(True)
    if Input[Row][Column] in ["L"]:
      return(False)
...
def Check(Row, Column):
  ...
  for One in range(-1, 2):
    for Two in range(-1, 2):
      if One == Two and One == 0:
        continue
      Total += First(Row, Column, One, Two)
  return(Total)

# print(Input)

# print(New)
New = [["." for  x in range(Width + 2)]]
while True:
  Changed = False
  NewInput = New + []
  ...
  for Row in range(1, len(Input) - 1):
    NewRow = ["."]
    for Column in range(1, len(Input[0]) - 1):
      CurrentPosition = Input[Row][Column]
      if CurrentPosition == ".":
        NewRow.append(".")
        continue
