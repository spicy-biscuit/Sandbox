Input = ["""eyr:2021 hgt:168cm hcl:#fffffd pid:180778832 byr:1923 ecl:amb iyr:2019 cid:241
...
def Valid(Fields, Values):
  ...
  Index = Fields.index("byr")
  Value = Values[Index]
  Value = int(Value)
  if Value < 1920 or Value > 2002:
    return(False)

  Index = Fields.index("iyr")
  Value = Values[Index]
  Value = int(Value)
  if Value < 2010 or Value > 2020:
    return(False)

  Index = Fields.index("eyr")
  Value = Values[Index]
  Value = int(Value)
  if Value < 2020 or Value > 2030:
    return(False)

  Index = Fields.index("hgt")
  Value = Values[Index]
  if len(Value) <= 2:
    return(False)
  Unit = Value[-2:]
  ...
  for x in range(len(Value)):
    if not Value[x].isdigit() and Value[x] not in ["a", "b", "c", "d", "e", "f"]:
      return(False)

  Index = Fields.index("ecl")
  Value = Values[Index]
  if Value not in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]:
    return(False)
    
  Index = Fields.index("pid")
  Value = Values[Index]
  if len(Value) != 9:
    return(False)
...
for Passport in Input:
  ...
  Fields = []
  Values = []
  for x in range(len(Passport)):
    if Passport[x].split(":")[0] == "cid":
      continue
    Fields.append(Passport[x].split(":")[0])
    Values.append(Passport[x].split(":")[1])

  Total += Valid(Fields, Values)
