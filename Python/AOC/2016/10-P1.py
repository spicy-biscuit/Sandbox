Input = ["""bot 135 gives low to bot 27 and high to bot 166
...
Bots = [[] for x in range(len(Input))]
Instructions = [[] for x in range(len(Input))]
Outputs = [[] for x in range(len(Input))]

for Instruction in Input:
  if Instruction[:5] == "value":
    Instruction = Instruction[6:]
    Value = int(Instruction[:Instruction.find(" ")])
    Bot = int(Instruction[Instruction.find("bot") + 4:])
    Bots[Bot].append(Value)
    # print(Bot, Value)
  else:
    LowOutput = Instruction[Instruction.find("low to") + 7:Instruction.find("and") - 1]
    HighOutput = Instruction[Instruction.find("high to") + 8:]
    Bot = int(Instruction[Instruction.find("bot") + 4:Instruction.find("gives") - 1])
    Instructions[Bot] = [LowOutput, HighOutput]
...
while True:
  ...
  for Bot in Bots:
    if len(Bot) != 2:
      continue
    Changed = True
    Instruction = Instructions[Bots.index(Bot)]
    Low = min(Bot)
    High = max(Bot)
    if [Low, High] == [17, 61]: #shift back
      print(Bots.index(Bot))
      Breakout = True
      break
    LowOutput = Instruction[0]
    HighOutput = Instruction[1]
    if HighOutput[0] == "b":
      Bots[int(HighOutput[HighOutput.find(" ") + 1:])].append(High)
    else:
      Outputs[int(HighOutput[HighOutput.find(" ") + 1:])].append(High)
    if LowOutput[0] == "b":
      Bots[int(LowOutput[LowOutput.find(" ") + 1:])].append(Low)
    else:
      Outputs[int(LowOutput[LowOutput.find(" ") + 1:])].append(Low)
    Bots[Bots.index(Bot)] = []
    break
  if Breakout:
    break
