...
def Function(String):
  ...
  String = String[String.find(" ") + 1:]
  Hour = int(String[:String.find(":")])
  Minute = int(String[String.find(":") + 1:String.find("]")])
  Guard = String.find("Guard") != -1
  # print(Month, Day, Hour, Minute, Guard)
  return(Minute + 100 * Hour + 10000 * Day + 1000000 * Month)

Input.sort(key = Function)

#find biggest guard
MaxGuard = 0
for Line in Input:
  if Line.find("Guard") != -1:
    Guard = int(Line[Line.find("#") + 1:Line.find("begins") - 1])
    if Guard > MaxGuard:
      MaxGuard = Guard

Asleep = [[0 for x in range(60)] for x in range(MaxGuard + 1)]
...
for Line in Input:
  ...
  if Line == "falls asleep": 
    # print(Guard, Time, Line)
    FellAsleep = Time
  if Line == "wakes up":
    # print(Guard, Time, Line)
    for x in range(FellAsleep, Time):
      Asleep[Guard][x] += 1

MaxSleepyMinutes = 0
SleepiestGuardID = 0

for x in range(MaxGuard):
  if sum(Asleep[x]) > MaxSleepyMinutes:
    MaxSleepyMinutes = sum(Asleep[x])
    SleepiestGuardID = x
MaxMinute = max(Asleep[SleepiestGuardID])
MaxMinute = Asleep[SleepiestGuardID].index(MaxMinute)
print(SleepiestGuardID * MaxMinute)