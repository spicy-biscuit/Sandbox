Input = ["""position=< 10253, -50152> velocity=<-1,  5>
...
for x in Input:
  ...
  Light.append(Row)
  Light.append(Column)
  RowVelocity = int(x[37])
  if x[36] == "-":
    RowVelocity *= -1
  ColumnVelocity = int(x[41])
  if x[40] == "-":
    ColumnVelocity *= -1
  ...
  # Light.append(Row)
  # Light.append(Column)
  # RowVelocity = int(x[28:30])
  # ColumnVelocity = int(x[32:34])

  Light.append(RowVelocity)
  Light.append(ColumnVelocity)
...
def Update(Tuple):
  ...
  Column = Tuple[1]
  RowVelocity = Tuple[2]
  ColumnVelocity = Tuple[3]
  Row += RowVelocity
  Column += ColumnVelocity
  return((Row, Column, RowVelocity, ColumnVelocity))

def BoundaryArea(AllTuples):
  MinRow = float('inf')
  MaxRow = 0
  MinColumn = float('inf')
  MaxColumn = 0
  for Light in AllTuples:
    Row = Light[0]
    Column = Light[1]
    if Row < MinRow:
      MinRow = Row
...
def AccessRows(Tuple):
  return(Tuple[0])
  
def AccessColumns(Tuple):
  return(Tuple[1])

MinimumBoundary = BoundaryArea(Lights)
TimeExpanding = 0
SavedItems = tuple([])
while True:
  Lights = list(Lights)
  ...
  Lights = tuple(Lights)
  Boundary = BoundaryArea(Lights)
  if Boundary < MinimumBoundary:
    MinimumBoundary = Boundary
    SavedItems = tuple(Lights)
    TimeExpanding = 0
  else:
    TimeExpanding += 1
  if TimeExpanding > 100:
    MinRow = min(SavedItems, key=AccessRows)[0]
    MaxRow = max(SavedItems, key=AccessRows)[0]
    MinColumn = min(SavedItems, key=AccessColumns)[1]
    MaxColumn = max(SavedItems, key=AccessColumns)[1]
    break

f = open("AOC/2018/10-P1-OUT.txt", "w")
