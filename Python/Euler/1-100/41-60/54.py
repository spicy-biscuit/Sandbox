...
def AssignValue(Hand):
    ...
    #Two Pair
    if max(Values) == 2 and Values.count(2) == 2:
        HandValue = 20000000 * 2
    ...
    #Straight
    for x in range(0, 10):
        if Values[x:x + 5] == [1, 1, 1, 1, 1]:
            HandValue = 20000000 * 4
            HandValue += 14 * x

    #Flush
    if max(Suits) == 5:
        HandValue = 20000000 * 5
        Copy = Values.copy()
        ...
        for x in range(5):
            for Index in reversed(range(13)):
                if Copy[Index] != 0:
                    Copy[Index] -= 1
                    HandValue += Index * (15 ** (5 - x))
                    break


    #Full house
    if Values.count(3) == 1 and Values.count(2) == 1:
        HandValue = 20000000 * 6
    ...
    if max(Suits) == 5:
        for x in range(0, 10):
            if Values[x:x + 5] == [1, 1, 1, 1, 1]:
                HandValue = 20000000 * 8
                HandValue += 14 * x
                break

    #Royal Flush
    if max(Suits) == 5 and Values[-5:] == [1, 1, 1, 1, 1]:
        HandValue = 20000000 * 9
...
for HAND in Poker:
    ...
    Player1 = HAND[:5]
    Player2 = HAND[5:]
    Player1 = AssignValue(Player1)
    Player2 = AssignValue(Player2)
    if Player1 > Player2:
        print("ONE")
        Out += 1
    else:
